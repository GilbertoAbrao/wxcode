...
Sistema de conversão incremental que transforma elementos WinDev/WebDev em código Python seguindo ordem topológica, gerando proposals OpenSpec que documentam decisões de mapeamento e servem de referência para conversões subsequentes.
...
- `openspec` CLI instalado e funcional
- MongoDB com elementos enriquecidos (topological_order, dependencies)
- LLM provider configurado (Anthropic, OpenAI, etc.)

## Non-functional Requirements

### Performance
- Geração de proposal SHOULD completar em menos de 60 segundos
- Carregamento de specs SHOULD ser cacheado durante sessão

### Reliability
- Falha em um elemento NÃO SHOULD impedir conversão dos próximos
- Status é persistido BEFORE operação para recovery

### Usability
- Output SHOULD indicar progresso claramente
- Erros SHOULD incluir sugestão de correção
