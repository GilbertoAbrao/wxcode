...
**No dependency analysis needed.** Wave numbers are pre-computed during `/wxcode:plan-phase`.
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="wxcode-executor", model="{executor_model}")
   ```
...
| `gaps_found` | Present gap summary, offer `/wxcode:plan-phase {phase} --gaps` |
...
- `/wxcode:verify-work {X}` — manual testing before planning
```

User runs `/wxcode:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/wxcode:execute-phase {X} --gaps-only`
...
1. Run `/wxcode:execute-phase {phase}` again
